...
func (s *Store) CreateObjectComment(objectType, dbName, tableName, columnName, commentText, createdBy string) (string, error) {
	id := uuid.NewString()
	...
	_, err := s.conn().Exec(
		`INSERT INTO gov_object_comments (id, object_type, database_name, table_name, column_name, comment_text, created_by, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		id,
...
func (s *Store) ListObjectComments(objectType, dbName, tableName, columnName string, limit int) ([]ObjectComment, error) {
	if limit <= 0 {
		limit = 100
	}
	if limit > 1000 {
	...
	query := fmt.Sprintf(
		`SELECT id, object_type, database_name, table_name, column_name, comment_text, created_by, created_at, updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&c.ID, &c.ObjectType, &c.DatabaseName, &c.TableName, &c.ColumnName,
...
func (s *Store) DeleteObjectComment(id string) error {
	res, err := s.conn().Exec(
		`DELETE FROM gov_object_comments WHERE id = ?`,
		id,
	)
	if err != nil {
...
func (s *Store) ListIncidents(status, severity string, limit int) ([]Incident, error) {
	if limit <= 0 {
		limit = 100
	}
	if limit > 1000 {
	...
	query := fmt.Sprintf(
		`SELECT id, source_type, source_ref, dedupe_key, title, severity, status, assignee, details, resolution_note,
...
func (s *Store) CreateIncident(sourceType, sourceRef, dedupeKey, title, severity, status, assignee, details, createdBy string) (string, error) {
	id := uuid.NewString()
	now := time.Now().UTC().Format(time.RFC3339)
	_, err := s.conn().Exec(
		`INSERT INTO gov_incidents
		 (id, source_type, source_ref, dedupe_key, title, severity, status, assignee, details, occurrence_count, first_seen_at, last_seen_at, created_by, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?)`,
		id,
...
func (s *Store) UpsertIncidentFromViolation(sourceRef, policyName, user, severity, detail string) (string, bool, error) {
	now := time.Now().UTC().Format(time.RFC3339)
...
func (s *Store) GetViolationByID(id string) (*PolicyViolation, error) {
	...
	err := row.Scan(&v.ID, &v.PolicyID, &queryLogID, &v.User, &v.ViolationDetail, &v.Severity, &v.DetectionPhase, &requestEndpoint, &v.DetectedAt, &v.CreatedAt, &v.PolicyName)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
