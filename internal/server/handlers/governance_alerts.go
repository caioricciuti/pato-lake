...
func (h *GovernanceHandler) CreateAlertChannel(w http.ResponseWriter, r *http.Request) {
	...
	id, err := h.DB.CreateAlertChannel(name, channelType, encrypted, isActive, session.Username)
	if err != nil {
...
func (h *GovernanceHandler) TestAlertChannel(w http.ResponseWriter, r *http.Request) {
	...
	if subject == "" {
		subject = "Patolake Alert Channel Test"
	}
	message := strings.TrimSpace(body.Message)
	if message == "" {
		message = "This is a test notification from Patolake."
	}
...
func (h *GovernanceHandler) CreateAlertRule(w http.ResponseWriter, r *http.Request) {
	...
	id, err := h.DB.CreateAlertRule(name, eventType, severityMin, enabled, cooldownSeconds, maxAttempts, body.SubjectTemplate, body.BodyTemplate, session.Username)
	if err != nil {
