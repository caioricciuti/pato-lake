...
func (h *DashboardsHandler) CreateDashboard(w http.ResponseWriter, r *http.Request) {
	...
	id, err := h.DB.CreateDashboard(name, strings.TrimSpace(body.Description), session.Username)
	if err != nil {
...
func (h *DashboardsHandler) CreatePanel(w http.ResponseWriter, r *http.Request) {
	...
	id, err := h.DB.CreatePanel(dashboardID, name, panelType, query, panelConfig, x, y, w2, h2)
	if err != nil {
...
func (h *DashboardsHandler) UpdatePanel(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.DB.UpdatePanel(panelID, name, panelType, query, panelConfig, x, y, pw, ph); err != nil {
...
// ExecutePanelQuery executes a SQL query through the DuckDB engine for a panel.
func (h *DashboardsHandler) ExecutePanelQuery(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		slog.Warn("Panel query failed", "error", err, "user", session.Username)
