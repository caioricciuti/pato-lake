...
func (h *QueryHandler) IngestUpload(w http.ResponseWriter, r *http.Request) {
	...
	if createTable {
		...
		createCtx, createCancel := context.WithTimeout(ctx, 45*time.Second)
		if _, err := h.Engine.Exec(createCtx, sql); err != nil {
			createCancel()
	...
	insertColumns, err := h.resolveInsertColumns(ctx, dbName, tableName, columns, createTable)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
	...
	rowsInserted, insertPreviewSQL, err := h.insertJSONEachRowBatches(ctx, dbName, tableName, insertColumns, dataset.Rows)
	if err != nil {
		message := err.Error()
		if insertPreviewSQL != "" {
...
func parseParquetDataset(payload []byte) (parsedUploadDataset, error) {
	tmp, err := os.CreateTemp("", "patolake-upload-*.parquet")
	if err != nil {
...
func (h *QueryHandler) resolveInsertColumns(
	...
	result, err := h.Engine.Execute(descCtx, fmt.Sprintf("DESCRIBE %s.%s", escapeIdentifier(databaseName), escapeIdentifier(tableName)))
	if err != nil {
		return nil, fmt.Errorf("failed to read table schema: %w", err)
	}

	// Convert [][]interface{} result to []map[string]interface{} using column meta
...
func (h *QueryHandler) insertJSONEachRowBatches(
	...
	for start := 0; start < len(rows); start += batchSize {
		...
		query, rowCount, err := buildInsertValuesQuery(databaseName, tableName, targetColumns, rows[start:end], start)
		if err != nil {
		...
		insertCtx, insertCancel := context.WithTimeout(ctx, 90*time.Second)
		_, err = h.Engine.Exec(insertCtx, query)
		insertCancel()
		if err != nil {
...
func buildInsertValuesQuery(
	...
	for rowIdx, source := range sourceRows {
		...
		for _, col := range columns {
			...
			values = append(values, formatSQLLiteral(coerced))
			if coerced != nil {
				hasData = true
			}
		}
...
// formatSQLLiteral converts a Go value into a SQL literal string suitable for
// embedding in an INSERT ... VALUES statement.
...
func coerceUploadValueForType(value interface{}, typeExpr string) (interface{}, error) {
	...
	baseType := normalizeType(typeExpr)
	if baseType == "" {
		baseType = "STRING"
	}
	...
	case strings.Contains(baseType, "INT"):
		...
		return parsed, nil
	case strings.Contains(baseType, "FLOAT") || strings.Contains(baseType, "DECIMAL") || strings.Contains(baseType, "DOUBLE"):
...
func normalizeType(typeExpr string) string {
	trimmed := strings.TrimSpace(typeExpr)
	if trimmed == "" {
		return ""
	}
	upper := strings.ToUpper(trimmed)
	// Strip Nullable(...) wrapper to get the base type for coercion.
	for strings.HasPrefix(upper, "NULLABLE(") && strings.HasSuffix(upper, ")") {
