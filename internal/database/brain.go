...
// GetBrainChatsByUser returns chats for a user.
func (db *DB) GetBrainChatsByUser(username string, includeArchived bool) ([]BrainChat, error) {
	query := `SELECT id, username, title, provider_id, model_id, archived, last_message_at, context_database, context_table, context_tables, created_at, updated_at FROM brain_chats WHERE username = ?`
...
func scanBrainChat(scanner interface {
	...
	var archived int
	if err := scanner.Scan(&c.ID, &c.Username, &c.Title, &providerID, &modelID, &archived, &lastMessageAt, &ctxDB, &ctxTable, &ctxTables, &c.CreatedAt, &c.UpdatedAt); err != nil {
...
func (db *DB) GetBrainChatByIDForUser(chatID, username string) (*BrainChat, error) {
	row := db.conn.QueryRow(`SELECT id, username, title, provider_id, model_id, archived, last_message_at, context_database, context_table, context_tables, created_at, updated_at FROM brain_chats WHERE id = ? AND username = ?`, chatID, username)
	var c BrainChat
	var providerID, modelID, lastMessageAt, ctxDB, ctxTable, ctxTables sql.NullString
	var archived int
	if err := row.Scan(&c.ID, &c.Username, &c.Title, &providerID, &modelID, &archived, &lastMessageAt, &ctxDB, &ctxTable, &ctxTables, &c.CreatedAt, &c.UpdatedAt); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
...
func (db *DB) CreateBrainChat(username, title, providerID, modelID, contextDatabase, contextTable, contextTables string) (string, error) {
	now := time.Now().UTC().Format(time.RFC3339)
	id := uuid.NewString()
	...
	if _, err := db.conn.Exec(
		`INSERT INTO brain_chats (id, username, title, provider_id, model_id, archived, last_message_at, context_database, context_table, context_tables, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, 0, NULL, ?, ?, ?, ?, ?)`,
		id, username, strings.TrimSpace(title), nullableString(providerID), nullableString(modelID), nullableString(contextDatabase), nullableString(contextTable), nullableString(contextTables), now, now,
	); err != nil {
