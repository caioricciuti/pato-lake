...
func (db *DB) GetModels() ([]Model, error) {
	rows, err := db.conn.Query(
		`SELECT id, name, description, target_database, materialization,
...
func (db *DB) CreateModel(name, description, targetDB, materialization, sqlBody, tableEngine, orderBy, createdBy string) (string, error) {
	id := uuid.NewString()
	...
	_, err := db.conn.Exec(
		`INSERT INTO models (id, name, description, target_database, materialization,
		                     sql_body, table_engine, order_by, status, created_by, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?, ?)`,
		id, name, description, targetDB, materialization,
...
func (db *DB) CreateModelRun(totalModels int, triggeredBy string) (string, error) {
	id := uuid.NewString()
	now := time.Now().UTC().Format(time.RFC3339)
	...
	_, err := db.conn.Exec(
		`INSERT INTO model_runs (id, status, total_models, started_at, triggered_by, created_at)
		 VALUES (?, 'running', ?, ?, ?, ?)`,
		id, totalModels, now, trigger, now,
	)
	if err != nil {
...
// GetModelRuns returns recent model runs.
func (db *DB) GetModelRuns(limit, offset int) ([]ModelRun, error) {
	rows, err := db.conn.Query(
		`SELECT id, status, total_models, succeeded, failed, skipped,
		        started_at, finished_at, triggered_by, created_at
		 FROM model_runs ORDER BY started_at DESC LIMIT ? OFFSET ?`,
		limit, offset,
	)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.Status, &r.Total, &r.Succeeded,
...
func (db *DB) GetModelRunByID(id string) (*ModelRun, error) {
	...
	var r ModelRun
	var finished, trigger sql.NullString
	err := row.Scan(&r.ID, &r.Status, &r.Total, &r.Succeeded,
...
// HasRunningModelRun checks if there's an active model run.
func (db *DB) HasRunningModelRun() (bool, error) {
	var count int
	err := db.conn.QueryRow(
		"SELECT COUNT(*) FROM model_runs WHERE status = 'running'",
	).Scan(&count)
	if err != nil {
...
func scanModelRow(row *sql.Row) (*Model, error) {
	var m Model
	var lastErr, lastRun, createdBy sql.NullString
	err := row.Scan(&m.ID, &m.Name, &m.Description,
		&m.TargetDatabase, &m.Materialization, &m.SQLBody, &m.TableEngine,
		&m.OrderBy, &m.Status, &lastErr, &lastRun, &createdBy,
...
// GetModelSchedules returns all model schedules.
func (db *DB) GetModelSchedules() ([]ModelSchedule, error) {
	rows, err := db.conn.Query(
		`SELECT id, anchor_model_id, cron, enabled, last_run_at, next_run_at,
...
func (db *DB) GetModelScheduleByAnchor(anchorModelID string) (*ModelSchedule, error) {
	row := db.conn.QueryRow(
		`SELECT id, anchor_model_id, cron, enabled, last_run_at, next_run_at,
		        last_status, last_error, created_by, created_at, updated_at
		 FROM model_schedules WHERE anchor_model_id = ?`,
		anchorModelID,
	)
	...
	err := row.Scan(&s.ID, &anchor, &s.Cron, &enabled,
...
func (db *DB) UpsertModelSchedule(anchorModelID, cron, nextRunAt, createdBy string) (string, error) {
	id := uuid.NewString()
	now := time.Now().UTC().Format(time.RFC3339)
	...
	_, err := db.conn.Exec(
		`INSERT INTO model_schedules
		 (id, anchor_model_id, cron, enabled, next_run_at, created_by, created_at, updated_at)
		 VALUES (?, ?, ?, 1, ?, ?, ?, ?)
		 ON CONFLICT(anchor_model_id)
		 DO UPDATE SET cron = excluded.cron, enabled = 1, next_run_at = excluded.next_run_at,
		               created_by = excluded.created_by, updated_at = excluded.updated_at`,
		id, anchorModelID, cron, nextRunAt, creator, now, now,
	)
	if err != nil {
...
func (db *DB) DeleteModelScheduleByAnchor(anchorModelID string) error {
	_, err := db.conn.Exec(
		"DELETE FROM model_schedules WHERE anchor_model_id = ?",
		anchorModelID,
	)
	if err != nil {
...
func scanModelSchedule(rows *sql.Rows) (ModelSchedule, error) {
	...
	var enabled int
	var anchor, lastRun, nextRun, lastStatus, lastErr, createdBy sql.NullString
	if err := rows.Scan(&s.ID, &anchor, &s.Cron, &enabled,
		&lastRun, &nextRun, &lastStatus, &lastErr, &createdBy,
