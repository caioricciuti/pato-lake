...
func (db *DB) GetSchedules() ([]Schedule, error) {
	rows, err := db.conn.Query(
		`SELECT id, name, saved_query_id, cron, timezone, enabled, timeout_ms,
...
func (db *DB) GetScheduleByID(id string) (*Schedule, error) {
	...
	var s Schedule
	var lastRun, nextRun, lastStatus, lastError, createdBy sql.NullString
	var enabled int

	err := row.Scan(&s.ID, &s.Name, &s.SavedQueryID, &s.Cron, &s.Timezone, &enabled, &s.TimeoutMs,
...
func (db *DB) CreateSchedule(name, savedQueryID, cron, timezone, createdBy string, timeoutMs int) (string, error) {
	id := uuid.NewString()
	...
	_, err := db.conn.Exec(
		`INSERT INTO schedules (id, name, saved_query_id, cron, timezone, enabled, timeout_ms, created_by, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?, ?)`,
		id, name, savedQueryID, cron, timezone, timeoutMs, creator, now, now,
	)
	if err != nil {
...
func scanSchedule(rows *sql.Rows) (Schedule, error) {
	...
	err := rows.Scan(&s.ID, &s.Name, &s.SavedQueryID, &s.Cron, &s.Timezone, &enabled, &s.TimeoutMs,
		&lastRun, &nextRun, &lastStatus, &lastError, &createdBy, &s.CreatedAt, &s.UpdatedAt)
	if err != nil {
