...
func (db *DB) GetPipelineByID(id string) (*Pipeline, error) {
	...
	err := row.Scan(&p.ID, &p.Name, &desc, &p.Status, &p.Config,
...
func (db *DB) CreatePipeline(name, description, createdBy string) (string, error) {
	id := uuid.NewString()
	...
	_, err := db.conn.Exec(
		`INSERT INTO pipelines (id, name, description, status, config, created_by, created_at, updated_at)
		 VALUES (?, ?, ?, 'draft', '{}', ?, ?, ?)`,
...
func scanPipeline(rows *sql.Rows) (Pipeline, error) {
	var p Pipeline
	var desc, createdBy, lastStarted, lastStopped, lastErr sql.NullString
	if err := rows.Scan(&p.ID, &p.Name, &desc, &p.Status, &p.Config,
		&createdBy, &lastStarted, &lastStopped, &lastErr,
