...
func (db *DB) CreateAlertEvent(eventType, severity, title, message string, payload interface{}, fingerprint, sourceRef string) (string, error) {
	id := uuid.NewString()
	now := time.Now().UTC().Format(time.RFC3339)
	...
	if _, err := db.conn.Exec(
		`INSERT INTO alert_events (id, event_type, severity, title, message, payload_json, fingerprint, source_ref, status, created_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'new', ?)`,
		id, strings.TrimSpace(eventType), strings.TrimSpace(severity),
...
func (db *DB) ListAlertEvents(limit int, eventType, status string) ([]AlertEvent, error) {
	...
	query := fmt.Sprintf(
		`SELECT id, event_type, severity, title, message, payload_json, fingerprint, source_ref, status, created_at, processed_at
		 FROM alert_events
		 WHERE %s
		 ORDER BY created_at DESC
	...
	for rows.Next() {
		var item AlertEvent
		var payloadJSON, fingerprint, sourceRef, processedAt sql.NullString
		if err := rows.Scan(
			&item.ID, &item.EventType, &item.Severity, &item.Title, &item.Message,
