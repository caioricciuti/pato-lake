...
func (db *DB) CreateAuditLog(params AuditLogParams) error {
	id := uuid.NewString()
	_, err := db.conn.Exec(
		`INSERT INTO audit_logs (id, action, username, details, ip_address)
		 VALUES (?, ?, ?, ?, ?)`,
		id, params.Action, params.Username, params.Details, params.IPAddress,
	)
	if err != nil {
...
func (db *DB) GetAuditLogsFiltered(limit int, timeRange, action, username, search string) ([]AuditLog, error) {
	...
	query.WriteString(`SELECT id, action, username, details, ip_address, created_at FROM audit_logs`)
	...
	for rows.Next() {
		var l AuditLog
		var username, details, ip sql.NullString
		if err := rows.Scan(&l.ID, &l.Action, &username, &details, &ip, &l.CreatedAt); err != nil {
