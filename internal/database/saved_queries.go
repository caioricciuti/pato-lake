...
func (db *DB) GetSavedQueries() ([]SavedQuery, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&q.ID, &q.Name, &desc, &q.Query, &createdBy, &q.CreatedAt, &q.UpdatedAt); err != nil {
...
func (db *DB) GetSavedQueryByID(id string) (*SavedQuery, error) {
	...
	var q SavedQuery
	var desc, createdBy sql.NullString
	err := row.Scan(&q.ID, &q.Name, &desc, &q.Query, &createdBy, &q.CreatedAt, &q.UpdatedAt)
...
func (db *DB) CreateSavedQuery(params CreateSavedQueryParams) (string, error) {
	...
	_, err := db.conn.Exec(
		`INSERT INTO saved_queries (id, name, description, query, created_by, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?)`,
		id, params.Name, desc, params.Query, createdBy, now, now,
	)
	if err != nil {
...
func (db *DB) UpdateSavedQuery(id, name, description, query string) error {
	now := time.Now().UTC().Format(time.RFC3339)
	...
	_, err := db.conn.Exec(
		`UPDATE saved_queries SET name = ?, description = ?, query = ?, updated_at = ? WHERE id = ?`,
		name, desc, query, now, id,
	)
	if err != nil {
