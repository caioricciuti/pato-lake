...
// Model names follow SQL identifier rules: [a-zA-Z_][a-zA-Z0-9_]*
var refPattern = regexp.MustCompile(`\$ref\(\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*\)`)

// modelNamePattern validates model names as valid SQL identifiers.
var modelNamePattern = regexp.MustCompile(`^[a-zA-Z_][a-zA-Z0-9_]*$`)

// ValidateModelName checks if a name is a valid SQL identifier.
