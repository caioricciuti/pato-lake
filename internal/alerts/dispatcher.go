...
func (d *Dispatcher) processDueJobs() {
	...
	for _, job := range jobs {
		...
		subject := renderTemplate(coalesce(job.RuleSubjectTemplate,
			fmt.Sprintf("[Patolake][%s][%s] %s", strings.ToUpper(job.EventSeverity), job.EventType, job.EventTitle)),
			job,
		)
...
func (d *Dispatcher) processDueDigests() {
	...
	for _, digest := range digests {
		...
		subject := fmt.Sprintf("[Patolake Digest][%s][%s] %d events", strings.ToUpper(digest.Severity), digest.EventType, digest.EventCount)
...
func renderDigestBody(digest database.AlertRouteDigestWithDetails) string {
	...
	var b strings.Builder
	b.WriteString("Patolake Alert Digest\n\n")
