# Patolake Makefile
# Single binary: server + embedded DuckDB + frontend
...
build-frontend:
	cd ui && bun install
	@cd ui && (PATOLAKE_VITE_MINIFY=true PATOLAKE_VITE_REPORT_COMPRESSED=false bun run build || \
		(echo "Frontend build was killed; retrying with low-memory profile (no minify)..." && \
		PATOLAKE_VITE_MINIFY=false PATOLAKE_VITE_REPORT_COMPRESSED=false bun run build))
