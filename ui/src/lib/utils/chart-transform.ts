...
// DuckDB date/time types
const DATE_TYPES = new Set([
  'DATE', 'TIMESTAMP', 'TIMESTAMP WITH TIME ZONE', 'TIMESTAMPTZ',
  'TIMESTAMP_S', 'TIMESTAMP_MS', 'TIMESTAMP_NS', 'TIME', 'TIMETZ',
])
...
export function isDateType(type: string): boolean {
  ...
  if (DATE_TYPES.has(upper)) return true
  return /^(DATE|TIME|TIMESTAMP)/i.test(type)
}

export function isNumericType(type: string): boolean {
  const upper = type.toUpperCase().replace(/^NULLABLE\(/, '').replace(/\)$/, '')
  if (NUMERIC_TYPES.has(upper)) return true
  return /^(TINY|SMALL|INTEGER|BIG|HUGE|U?INT|FLOAT|DOUBLE|DECIMAL)/i.test(upper)
}
