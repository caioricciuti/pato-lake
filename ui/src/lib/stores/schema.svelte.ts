...
export async function loadDatabases(): Promise<void> {
  ...
  try {
    ...
    databases = (res.databases ?? []).map(d => ({ name: d.name }))
  } catch {
    databases = []
  } finally {
    loading = false
...
export async function loadTables(dbName: string): Promise<void> {
  ...
  try {
    const res = await apiGet<{ tables: Array<{ name: string; type: string }> }>(`/api/query/tables?database=${encodeURIComponent(dbName)}`)
    const tables: Table[] = (res.tables ?? []).map(t => ({ name: t.name, table_type: t.type }))
    databases = databases.map(db => {
      if (db.name !== dbName) return db
