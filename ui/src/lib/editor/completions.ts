...
// DuckDB-specific keywords not typically returned by the server cache
const DUCKDB_KEYWORDS = [
  'PIVOT', 'UNPIVOT', 'QUALIFY', 'SAMPLE', 'TABLESAMPLE',
  'ATTACH', 'DETACH', 'COPY', 'INSTALL', 'LOAD', 'PRAGMA',
  'DESCRIBE', 'SUMMARIZE', 'EXCLUDE', 'REPLACE', 'COLUMNS',
  'STRUCT', 'LIST', 'MAP', 'UNION TYPE',
]

function buildKeywordCompletions(): Completion[] {
  const serverKw = (cachedKeywords ?? []).map((kw) => ({
    label: kw,
    type: 'keyword' as const,
    boost: 4,
  }))
  const serverLabels = new Set(serverKw.map((k) => k.label.toUpperCase()))
  const duckKw = DUCKDB_KEYWORDS
    .filter((kw) => !serverLabels.has(kw.toUpperCase()))
    .map((kw) => ({
      label: kw,
      type: 'keyword' as const,
      boost: 4,
    }))
  return [...serverKw, ...duckKw]
}
...
function buildSnippetCompletions(): Completion[] {
  return [
    ...
    snippetCompletion("read_parquet('${path}')", {
      label: "read_parquet()",
      type: 'snippet',
      detail: 'Read Parquet files',
      boost: 22,
    }),
    snippetCompletion("read_csv_auto('${path}')", {
      label: "read_csv_auto()",
      type: 'snippet',
      detail: 'Read CSV files',
      boost: 22,
    }),
    snippetCompletion("read_json_auto('${path}')", {
      label: "read_json_auto()",
      type: 'snippet',
      detail: 'Read JSON files',
      boost: 22,
    }),
    snippetCompletion('unnest(${list})', {
      label: 'unnest()',
      type: 'snippet',
      detail: 'Expand list to rows',
      boost: 20,
    }),
    snippetCompletion("list_aggregate(${list}, '${func}')", {
      label: "list_aggregate()",
      type: 'snippet',
      detail: 'Aggregate over list',
      boost: 20,
    }),
    snippetCompletion('struct_pack(${k} := ${v})', {
      label: 'struct_pack()',
      type: 'snippet',
      detail: 'Create struct',
      boost: 20,
    }),
    snippetCompletion("COPY ${table} TO '${file.parquet}'", {
      label: "COPY … TO",
      type: 'snippet',
      detail: 'Export data',
      boost: 18,
    }),
    snippetCompletion("ATTACH '${path}' AS ${name}", {
      label: "ATTACH … AS",
      type: 'snippet',
      detail: 'Attach database',
      boost: 18,
    }),
    snippetCompletion('PIVOT ${table} ON ${column} USING ${agg}', {
      label: 'PIVOT … ON … USING',
      type: 'snippet',
      detail: 'Pivot table',
      boost: 18,
    }),
    snippetCompletion('UNPIVOT ${table} ON ${columns} INTO NAME ${name_col} VALUE ${value_col}', {
      label: 'UNPIVOT … ON … INTO',
      type: 'snippet',
      detail: 'Unpivot table',
      boost: 18,
    }),
    snippetCompletion('SUMMARIZE ${table_name}', {
      label: 'SUMMARIZE',
      type: 'snippet',
      detail: 'Quick stats',
      boost: 16,
    }),
    snippetCompletion('DESCRIBE ${table_name}', {
      label: 'DESCRIBE',
      type: 'snippet',
      detail: 'Show schema',
      boost: 16,
    }),
    snippetCompletion('FROM ${table_name} SELECT *', {
      label: 'FROM … SELECT *',
      type: 'snippet',
      detail: "DuckDB FROM-first syntax",
      boost: 28,
    }),
  ]
}
