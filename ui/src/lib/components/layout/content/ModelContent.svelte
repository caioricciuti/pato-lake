<script lang="ts">
  ...
  // Split panel
  const savedSplit = parseFloat(localStorage.getItem('patolake-model-split-percent') ?? '60')
  ...
  function onDragEnd() {
    ...
    localStorage.setItem('patolake-model-split-percent', String(splitPercent))
  }
</script>
...
<div class="flex flex-col h-full overflow-hidden" bind:this={containerEl}>
  ...
  <div class="flex items-center gap-2 px-3 py-2 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 shrink-0 flex-wrap">
    <input
      ...
      class="text-sm font-semibold bg-transparent border-0 border-b border-transparent hover:border-gray-300 dark:hover:border-gray-600 focus:border-yellow-400 focus:outline-none text-gray-800 dark:text-gray-200 px-1 py-0.5 min-w-[120px] max-w-[200px]"
    ...
    <input
      ...
      class="text-xs bg-transparent border border-gray-300 dark:border-gray-600 rounded px-1.5 py-0.5 w-24 text-gray-700 dark:text-gray-300 focus:border-yellow-400 focus:outline-none"
    />
    <span class="text-gray-300 dark:text-gray-600">|</span>
    ...
    <button
      onclick={handleSave}
      disabled={saving}
      class="flex items-center gap-1 text-[10px] px-2.5 py-1 rounded bg-yellow-500 text-white hover:bg-yellow-600 disabled:opacity-50 transition-colors font-medium"
    >
