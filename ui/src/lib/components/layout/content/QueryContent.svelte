<script lang="ts">
  ...
  import { formatSQL, explainQuery, fetchQueryPlan } from '../../../api/query'
  ...
  import QueryHistory from '../../editor/QueryHistory.svelte'
  ...
  const savedSplit = parseFloat(localStorage.getItem('patolake-split-percent') ?? '40')
  ...
  async function handleLoadPlan() {
    ...
    try {
      ...
      if (!planNodes.length && !planLines.length) {
        planError = 'No plan rows returned by DuckDB'
      }
    } catch (e: any) {
      planError = e.message
    } finally {
  ...
  function onDragEnd() {
    ...
    localStorage.setItem('patolake-split-percent', String(splitPercent))
  }
</script>
