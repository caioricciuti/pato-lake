<script lang="ts">
  ...
  import { Table2, HardDrive, RefreshCw, Copy, Check, ChevronDown, Braces, FileSpreadsheet, FileText, Code2 } from 'lucide-svelte'
  ...
  const lightTheme = EditorView.theme({
    ...
    '.cm-selectionBackground': { backgroundColor: '#fef9c3 !important' },
    '&.cm-focused .cm-selectionBackground': { backgroundColor: '#fef08a !important' },
    '.cm-cursor': { borderLeftColor: '#1f2126' },
    '.cm-matchingBracket': { backgroundColor: '#fef9c3', outline: '1px solid #facc15' },
  }, { dark: false })

  const darkTheme = EditorView.theme({
    '&': { backgroundColor: '#17181d', color: '#f3f4f6' },
    '.cm-gutters': { backgroundColor: '#1d1f25', borderRight: '1px solid #3f434c', color: '#a5a8b2' },
    '.cm-activeLine': { backgroundColor: 'rgba(234,179,8,0.1)' },
    '.cm-activeLineGutter': { backgroundColor: 'rgba(234,179,8,0.16)' },
    '.cm-selectionBackground': { backgroundColor: 'rgba(250,204,21,0.32) !important' },
    '.cm-matchingBracket': { backgroundColor: 'rgba(234,179,8,0.2)', outline: '1px solid rgba(250,204,21,0.9)' },
  ...
  const darkHighlight = HighlightStyle.define([
    { tag: [t.keyword, t.operatorKeyword, t.controlKeyword, t.definitionKeyword, t.moduleKeyword], color: '#facc15', fontWeight: '600' },
    { tag: [t.function(t.variableName), t.function(t.propertyName)], color: '#fde047' },
    ...
    { tag: [t.number, t.integer, t.float, t.atom], color: '#fde047' },
  ...
  function formatCreateTableSQL(raw: string): string {
    ...
    s = s.replace(/\(\s*(")/g, '(\n  $1')
    // Each column on its own line (comma followed by identifier)
    s = s.replace(/,\s*(")/g, ',\n  $1')
    return s
  }
  ...
  const metrics = $derived.by(() => {
    ...
    return [
      { label: 'Estimated Size', value: formatBytes(Number(tableInfo.estimated_size ?? 0)), icon: HardDrive, color: 'text-ch-green' },
      { label: 'Columns', value: formatNumber(Number(tableInfo.column_count ?? 0)), icon: Columns3, color: 'text-ch-orange' },
    ]
  })
