<script lang="ts">
  ...
  const navItems: NavItem[] = [
    ...
    { type: 'ingest', label: 'Ingest', icon: ArrowDownToLine },
    { type: 'models', label: 'Models', icon: Boxes },
    { type: 'notebooks', label: 'Notebooks', icon: BookOpen },
    ...
    { type: 'external', label: 'Patolake Docs', icon: ExternalLink, href: 'https://patolake.com/docs' },
  ]
  ...
  const savedCollapsed = localStorage.getItem('patolake-sidebar-collapsed') === 'true'
  const savedWidth = parseInt(localStorage.getItem('patolake-sidebar-width') ?? String(DEFAULT_WIDTH), 10)
  const savedMenuCollapsed = localStorage.getItem('patolake-sidebar-menu-collapsed') === 'true'
  ...
  function toggleCollapse() {
    collapsed = !collapsed
    localStorage.setItem('patolake-sidebar-collapsed', String(collapsed))
  }

  function toggleMenuCollapsed() {
    menuCollapsed = !menuCollapsed
    localStorage.setItem('patolake-sidebar-menu-collapsed', String(menuCollapsed))
  }
  ...
  function onDragMove(e: MouseEvent) {
    ...
    if (newWidth < COLLAPSE_THRESHOLD) {
      collapsed = true
      localStorage.setItem('patolake-sidebar-collapsed', 'true')
    } else {
      collapsed = false
      localStorage.setItem('patolake-sidebar-collapsed', 'false')
  ...
  function onDragEnd() {
    ...
    document.removeEventListener('mousemove', onDragMove)
    document.removeEventListener('mouseup', onDragEnd)
    localStorage.setItem('patolake-sidebar-width', String(sidebarWidth))
  }
</script>
  ...
  >
    {#if collapsed}
            ...
            <button
              class="p-2 rounded transition-colors
                {isNavItemActive(item.type)
                  ? 'text-ch-orange bg-yellow-100/70 dark:bg-yellow-500/18'
      ...
      <div class="flex flex-col items-center py-2 gap-1 border-t border-gray-200 dark:border-gray-800">
        {#if session}
          <span
            class="w-2 h-2 rounded-full my-1 bg-green-500"
            title="Connected"
          ></span>
        {/if}
        <button
                ...
                {:else}
                  <button
                    class="flex items-center gap-2.5 w-full px-3.5 py-2 text-[13px] font-medium transition-colors
                      {isNavItemActive(item.type)
                        ? 'text-ch-orange bg-yellow-100/70 dark:bg-yellow-500/18 border-l-2 border-ch-orange'
                        : 'text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-200/40 dark:hover:bg-gray-800/40'}"
                    onclick={() => openSingletonTab(item.type, item.label)}
                  >
                    <item.icon size={15} />
            ...
            <div class="px-3.5 py-2.5 border-t border-gray-200 dark:border-gray-800 shrink-0">
              {#if session}
                <div class="flex items-center gap-2 mb-1.5">
                  ...
                  <span class="text-[13px] font-semibold text-gray-700 dark:text-gray-300 truncate">Patolake</span>
                  <span
                    class="w-1.5 h-1.5 rounded-full shrink-0 bg-green-500"
                    title="Connected"
                  ></span>
                  <span class="text-[10px] text-gray-400 truncate">{session.user.username}</span>
                </div>
              {/if}
              <div class="flex items-center gap-1">
                {#if session?.version}
                  <span class="text-[10px] text-gray-400 dark:text-gray-600 mr-auto">patolake {session.version}</span>
                {/if}
