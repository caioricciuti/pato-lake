<script lang="ts">
  ...
  let columnSeed = 0;
  function newColumnDraft(name = "", type = "VARCHAR"): TableColumnDraft {
    columnSeed += 1;
    return {
      id: `col-${columnSeed}`,
      name,
  ...
  let createTableForm = $state({
    ...
    comment: "",
    columns: [
      newColumnDraft("id", "INTEGER"),
      newColumnDraft("created_at", "TIMESTAMP"),
    ],
  });
  ...
  function getTableType(tableType?: string): "view" | "table" {
  ...
  function getTableTypeLabel(tableType?: string): string {
    const t = getTableType(tableType);
    if (t === "view") return "VIEW";
    return "";
  }
  ...
  function queryDatabase(database: string) {
    openQueryTab(`SHOW TABLES FROM "${database.replace(/"/g, '""')}"`);
    closeMenu();
  }

  function queryTable(database: string, table: string) {
    openQueryTab(`SELECT *\nFROM "${database.replace(/"/g, '""')}"."${table.replace(/"/g, '""')}"\nLIMIT 1000`);
    closeMenu();
  }
  ...
  function getExplorerMenuItems(): ContextMenuItem[] {
    ...
    return [
      ...
      { id: "sep1", separator: true },
      {
        ...
        icon: Trash2,
        danger: true,
        disabled: isSystemDatabase(target.database),
                  ...
                  >
                    ...
                    {#if getTableTypeLabel(table.table_type)}
                      <span class="ml-1 text-[9px] font-medium px-1 py-0.5 rounded bg-gray-200/60 dark:bg-gray-700/60 text-gray-500 dark:text-gray-400 shrink-0"
                        >{getTableTypeLabel(table.table_type)}</span
                      >
                    {/if}
                    {#if table.loading}
                      <Spinner size="sm" class="ml-auto" />
      ...
      <div class="flex items-center justify-between gap-2">
        <div>
          ...
          <p class="text-[11px] text-gray-500">
            Type is selected from DuckDB data types (parametric families are
