<script lang="ts">
  ...
  import { getDisplayType } from '../../utils/duckdb-types'
  ...
  const badgeColors: Record<string, string> = {
    number: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300',
  ...
  <table class="w-full text-sm border-collapse">
    <thead class="sticky top-0 z-10 bg-gray-50 dark:bg-gray-900">
      <tr class="border-b border-gray-300 dark:border-gray-700">
        ...
        <th class="px-3 py-1.5 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">DuckDB Type</th>
