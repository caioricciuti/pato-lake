<script lang="ts">
  ...
  function buildBaseWidths(columns: ColumnMeta[], rows: unknown[][]): number[] {
    ...
    return columns.map((col, ci) => {
      ...
      if (/^(DATE|TIME|TIMESTAMP)/i.test(col.type)) width = Math.max(width, 140)
      if (/^(TINY|SMALL|INTEGER|BIG|HUGE|U?INT|FLOAT|DOUBLE|DECIMAL)/i.test(col.type)) width = Math.max(width, 124)
      if (/^(UUID|BLOB)/i.test(col.type)) width = Math.max(width, 180)
    ...
    {#if rowCount > 0}
      <tbody>
        ...
        {#each visibleRows as row, vi (startIdx + vi)}
          ...
          <tr
            class="group h-[34px] border-b border-gray-100 dark:border-gray-900 hover:bg-yellow-50/70 dark:hover:bg-ch-orange/8 cursor-default
              {absIdx % 2 === 1 ? 'bg-gray-50 dark:bg-gray-900' : 'bg-white dark:bg-gray-950'}
              {selectedRow === absIdx ? 'bg-yellow-100 dark:bg-yellow-950' : ''}"
          ...
          >
            <td
              class="sticky left-0 z-[2] px-2.5 text-right text-xs font-semibold text-gray-700 dark:text-gray-200 border-r border-gray-200 dark:border-gray-800 tabular-nums select-none
                {absIdx % 2 === 1 ? 'bg-gray-50 dark:bg-gray-900' : 'bg-white dark:bg-gray-950'}
                {selectedRow === absIdx ? 'bg-yellow-100 dark:bg-yellow-950' : ''}
                group-hover:bg-yellow-50 dark:group-hover:bg-yellow-900/60"
