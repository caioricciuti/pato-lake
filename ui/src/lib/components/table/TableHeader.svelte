<script lang="ts">
  ...
  import { getDisplayType, isRightAligned } from '../../utils/duckdb-types'
  ...
  function typeTone(type: string): string {
    switch (getDisplayType(type)) {
      case 'number':
        return 'text-yellow-700 dark:text-yellow-300 bg-yellow-100/70 dark:bg-yellow-500/15 border-yellow-200/70 dark:border-yellow-500/25'
      case 'date':
