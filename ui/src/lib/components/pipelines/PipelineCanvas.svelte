...
<div class="flex flex-1 min-h-0">
  ...
  <div class="w-44 border-r border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-950 p-2 overflow-auto">
    ...
    {#each SOURCE_NODE_TYPES as source}
      ...
      <div
        class="flex items-center gap-2 px-2 py-1.5 mb-1 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 cursor-grab hover:border-yellow-300 dark:hover:border-yellow-700 transition-colors text-xs"
        draggable="true"
    ...
    {#each SINK_NODE_TYPES as sink}
      <div
        class="flex items-center gap-2 px-2 py-1.5 mb-1 rounded-lg border border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-900/20 cursor-grab hover:border-yellow-400 dark:hover:border-yellow-600 transition-colors text-xs"
        draggable="true"
        ondragstart={(e: DragEvent) => onDragStart(e, sink.type)}
        role="button"
        tabindex={0}
      >
        <Database size={14} class="text-yellow-500 shrink-0" />
        <div class="min-w-0">
  ...
  >
    <SvelteFlow
      ...
      defaultEdgeOptions={{ animated: true, style: 'stroke: #eab308; stroke-width: 2;' }}
    >
      <Controls />
      <Background gap={16} />
      <MiniMap />
