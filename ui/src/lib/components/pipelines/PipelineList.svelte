  ...
  <div class="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-800">
    ...
    <div class="flex items-center gap-2">
      <input
        ...
        class="h-8 w-52 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 text-xs text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-yellow-500"
      />
    ...
    {:else if filtered.length === 0}
      <div class="text-center py-12 text-gray-500">
        ...
        {:else}
          ...
          <p class="text-xs text-gray-400 dark:text-gray-600">Create a pipeline to start ingesting data into DuckDB</p>
        {/if}
      </div>
    {:else}
      <div class="grid gap-3 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
        {#each filtered as pipeline (pipeline.id)}
          <div
            class="group text-left rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 hover:border-yellow-300 dark:hover:border-yellow-700 hover:shadow-sm transition-all cursor-pointer"
