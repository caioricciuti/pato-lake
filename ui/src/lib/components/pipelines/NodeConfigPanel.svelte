  ...
  <div class="flex-1 overflow-auto p-3 space-y-3">
    ...
    <div>
      ...
      <input
        ...
        class="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-2.5 py-1.5 text-xs text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-yellow-500"
      />
    </div>
      ...
      <div>
        ...
        {:else if field.type === 'textarea'}
          <textarea
            ...
            rows={3}
            class="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-2.5 py-1.5 text-xs text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-yellow-500 resize-none font-mono"
          ></textarea>
        {:else if field.type === 'toggle'}
          <button
            aria-label={field.label}
            class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors {
              getFieldValue(field) ? 'bg-yellow-500' : 'bg-gray-300 dark:bg-gray-700'
            }"
            onclick={() => {
          ...
          {#if field.key === 'auth_enabled' && getFieldValue(field) && localConfig['auth_token']}
            <div class="mt-2">
              <label class="flex items-center gap-1 text-[10px] font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">
                <Shield size={10} class="text-yellow-500" />
                Bearer Token
              </label>
