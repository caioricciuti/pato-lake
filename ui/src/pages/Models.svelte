<script lang="ts">
  ...
  // Info banner
  let infoDismissed = $state(localStorage.getItem('patolake-pipeline-info-dismissed') === '1')

  function dismissInfo() {
    infoDismissed = true
    localStorage.setItem('patolake-pipeline-info-dismissed', '1')
  }
  ...
  async function loadDAG() {
    try {
      ...
      dagEdges = dagEdgesRaw.map((e: DAGEdge) => ({
        ...
        target: e.target,
        animated: true,
        style: 'stroke: #eab308; stroke-width: 2px;',
      }))
    } catch (e: unknown) {
...
<div class="flex flex-col h-full overflow-hidden">
  ...
  <div class="flex items-center gap-3 px-4 py-2.5 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-950 shrink-0">
    <Boxes size={18} class="text-yellow-500 shrink-0" />
    ...
    <button
      ...
      class="flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded text-gray-600 dark:text-gray-300 hover:text-yellow-500 hover:bg-yellow-50 dark:hover:bg-yellow-900/20 disabled:opacity-40 transition-colors"
    ...
    <button
      onclick={openHistory}
      class="flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded text-gray-600 dark:text-gray-300 hover:text-yellow-500 hover:bg-yellow-50 dark:hover:bg-yellow-900/20 transition-colors"
      title="Run history"
    >
    ...
    <button
      ...
      class="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded bg-yellow-500 text-white hover:bg-yellow-600 transition-colors font-medium"
    >
  ...
  <div class="flex-1 min-h-0 overflow-auto">
    ...
    {:else if models.length === 0}
      <div class="flex flex-col items-center justify-center h-full gap-4 text-gray-400 dark:text-gray-500">
        ...
        <button
          onclick={handleCreate}
          class="flex items-center gap-1.5 text-xs px-4 py-2 rounded bg-yellow-500 text-white hover:bg-yellow-600 transition-colors font-medium"
        >
          ...
          {#if pipelineModels.length > 0}
            <div class="rounded-lg border border-gray-200 dark:border-gray-700/80 bg-white dark:bg-gray-900/30 overflow-hidden">
              ...
              <div class="flex items-center gap-2.5 px-4 py-2.5 bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700/60">
                <Boxes size={14} class="text-yellow-400 shrink-0" />
                ...
                <button
                  ...
                  class="flex items-center gap-1 text-[11px] px-2 py-1 rounded transition-colors
                    {pipeline.schedule
                      ? 'text-yellow-600 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-900/20 font-mono'
              ...
              <div class="p-3 grid gap-3 grid-cols-[repeat(auto-fill,minmax(240px,1fr))]">
                {#each pipelineModels as model (model.id)}
                  ...
                  <div
                    class="group relative flex flex-col gap-2 p-3.5 rounded-lg border border-gray-200 dark:border-gray-700/80 bg-white dark:bg-gray-900/50 hover:border-yellow-300 dark:hover:border-yellow-500/40 hover:shadow-sm cursor-pointer transition-all"
                  ...
                  >
                    <div class="flex items-center gap-2">
                      {#if model.materialization === 'table'}
                        <Table2 size={14} class="text-yellow-400 shrink-0" />
                      {:else}
                    ...
                    {#if upstreamMap.get(model.id)?.length}
                      <div class="flex items-center gap-1.5 flex-wrap">
                        <GitBranch size={11} class="text-yellow-400 shrink-0" />
                        {#each upstreamMap.get(model.id)! as dep}
                          <span class="text-[10px] px-1.5 py-0.5 rounded-full bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400 font-medium">{dep}</span>
                        {/each}
                      </div>
                    {/if}
                    {#if model.last_error}
...
{#if showDAG}
  ...
  <div class="fixed inset-0 z-50 flex flex-col bg-white dark:bg-gray-950" role="dialog" tabindex="-1">
    <div class="flex items-center gap-3 px-4 py-3 border-b border-gray-200 dark:border-gray-700 shrink-0">
      <GitBranch size={18} class="text-yellow-500" />
...
{#if showHistory}
  ...
  <div class="fixed inset-0 z-50 flex flex-col bg-white dark:bg-gray-950" role="dialog" tabindex="-1">
    <div class="flex items-center gap-3 px-4 py-3 border-b border-gray-200 dark:border-gray-700 shrink-0">
      <History size={18} class="text-yellow-500" />
      ...
      <div class="p-4 space-y-4">
        ...
        <div>
          ...
          <input
            ...
            placeholder="0 */6 * * *"
            class="w-full text-sm border border-gray-300 dark:border-gray-600 rounded px-3 py-1.5 bg-transparent text-gray-800 dark:text-gray-200 focus:border-yellow-400 focus:outline-none font-mono"
          />
        </div>
        ...
        <div class="flex items-center gap-2 pt-2">
          <button
            ...
            class="flex items-center gap-1 text-xs px-3 py-1.5 rounded bg-yellow-500 text-white hover:bg-yellow-600 disabled:opacity-50 transition-colors font-medium"
          >
