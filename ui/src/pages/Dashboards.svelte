<script lang="ts">
  ...
  let dashboardTimeRange = $state(localStorage.getItem('patolake-dashboard-time-range') ?? '1h')
  ...
  function handleTimeRangeChange(nextRange: string) {
    ...
    localStorage.setItem('patolake-dashboard-time-range', nextRange)
    runAllPanelQueries()
  }
    ...
    <div class="flex-1 overflow-auto p-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {#each { length: 6 } as _}
            <div class="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-4 space-y-3">
              <Skeleton width="70%" height="1rem" rounded="rounded" />
              <Skeleton width="50%" height="0.75rem" rounded="rounded" />
              <div class="flex items-center gap-3">
                <Skeleton width="60px" height="0.7rem" rounded="rounded" />
                <Skeleton width="100px" height="0.7rem" rounded="rounded" />
              </div>
            </div>
          {/each}
        </div>
      {:else if dashboards.length === 0}
        <div class="text-center py-12 text-gray-500">
          ...
          <p class="text-xs text-gray-400 dark:text-gray-600">Create a dashboard to visualize your DuckDB data</p>
        </div>
      {:else}
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
