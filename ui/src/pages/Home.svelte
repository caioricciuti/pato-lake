<script lang="ts">
  ...
  const quickLinks: QuickLink[] = [
    ...
    },
    {
      id: 'brain',
      title: 'Brain',
      description: 'AI assistant for DuckDB workflows',
      icon: Brain,
  ...
  const resources: ResourceLink[] = [
    ...
    {
      id: 'project-releases',
      title: 'Patolake Releases',
      description: 'Binary downloads and release notes',
      href: 'https://github.com/caioricciuti/pato-lake/releases',
    },
    {
      id: 'project-issues',
      title: 'Report Issue',
      description: 'Open bug reports and feature requests',
      href: 'https://github.com/caioricciuti/pato-lake/issues',
    },
  ]
  ...
  function openTab(tab: Tab): void {
    ...
    if (tab.type === 'table') {
      openQueryTab(`SELECT *\nFROM "${tab.database.replace(/"/g, '""')}"."${tab.table.replace(/"/g, '""')}"\nLIMIT 1000`)
      return
    }
    if (tab.type === 'database') {
      openQueryTab(`SHOW TABLES FROM "${tab.database.replace(/"/g, '""')}"`)
  ...
  <div class="mx-auto w-full max-w-6xl p-5 lg:p-7 space-y-5">
    <section class="surface-card rounded-2xl border border-gray-200/80 dark:border-gray-800/80 p-5 lg:p-6">
      <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div class="space-y-1">
          <div class="inline-flex items-center gap-2 rounded-full border border-yellow-300/45 dark:border-yellow-700/45 bg-yellow-100/60 dark:bg-yellow-500/12 px-3 py-1 text-xs font-medium text-ch-orange">
            <Home size={12} />
            Workspace Home
          </div>
        ...
        <button
          class="inline-flex items-center justify-center gap-2 rounded-lg bg-ch-orange px-4 py-2 text-sm font-semibold text-white hover:bg-yellow-500 transition-colors"
    ...
    <section class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3.5">
      {#each quickLinks as item (item.id)}
        <button
          class="surface-card rounded-xl border border-gray-200/70 dark:border-gray-800/70 px-4 py-3.5 text-left hover:border-ch-orange/45 hover:bg-yellow-50/35 dark:hover:bg-yellow-500/8 transition-colors"
          onclick={item.run}
        >
      ...
      {:else}
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {#each recentTabs as tab (tab.id)}
            ...
            <button
              class="surface-card rounded-xl border border-gray-200/75 dark:border-gray-800/75 px-4 py-3 text-left hover:border-ch-orange/45 hover:bg-yellow-50/30 dark:hover:bg-yellow-500/8 transition-colors"
              onclick={() => openTab(tab)}
            >
