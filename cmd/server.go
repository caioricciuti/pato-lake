...
var serverCmd = &cobra.Command{
	Use:   "server",
	Short: "Start the Patolake server",
	Long:  "Start the Patolake HTTP server that serves the API, frontend, and DuckDB query engine.",
...
var serverStopCmd = &cobra.Command{
	Use:   "stop",
	Short: "Stop the Patolake server",
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if stopped {
			fmt.Println("Patolake server stopped")
		}
		return nil
	},
}
...
var serverStatusCmd = &cobra.Command{
	Use:   "status",
	Short: "Show Patolake server status",
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if running {
			fmt.Printf("Patolake server is running (PID %d)\n", pid)
		...
		if isTCPPortOpen(addr) {
			fmt.Printf("Patolake server PID file not found, but port %d is in use.\n", serverPort)
			fmt.Printf("Another process may be listening on %s.\n", addr)
			return nil
		}

		fmt.Println("Patolake server is not running")
		return nil
	},
}
...
var serverRestartCmd = &cobra.Command{
	Use:   "restart",
	Short: "Restart the Patolake server",
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if restartDetach {
			...
			fmt.Printf("Patolake server restarted in background (PID %d)\n", pid)
			if logPath != "" {
				fmt.Printf("Logs: %s\n", logPath)
			}
			return nil
...
func init() {
	...
	pf.StringVar(&serverDuckDBPath, "duckdb-path", "", "Path to persistent DuckDB database file")
	pf.StringVarP(&serverConfig, "config", "c", "", "Path to config file")
	pf.StringVar(&serverPIDFile, "pid-file", "patolake-server.pid", "Path to server PID file")
...
func runServer(cmd *cobra.Command) error {
	if serverDetach {
		...
		fmt.Printf("Patolake server started in background (PID %d)\n", pid)
		if logPath != "" {
			fmt.Printf("Logs: %s\n", logPath)
		}
		return nil
	...
	if cmd.Flags().Changed("duckdb-path") {
		cfg.DuckDBPath = strings.TrimSpace(serverDuckDBPath)
	}
	cfg.DevMode = devMode
	...
	slog.Info("Starting Patolake server",
	...
	// Initialize SQLite (app metadata)
	...
	if err := db.BootstrapAdmin(cfg.AdminUsername, cfg.AdminPassword); err != nil {
		slog.Warn("Admin bootstrap", "error", err)
	}
	...
	engine, err := duckdb.NewEngine(duckdb.Config{
		...
		Extensions:  cfg.DuckDBExtensions,
	})
	if err != nil {
	...
	ver, _ := engine.Version(context.Background())
	slog.Info("DuckDB engine ready", "version", ver, "path", cfg.DuckDBPath)

	// Reattach any user-created databases from the data directory
	engine.AttachExistingDatabases(filepath.Dir(cfg.DuckDBPath), cfg.DuckDBPath)
	...
	srv := server.New(cfg, db, engine, FrontendFS)
...
func buildServerStartArgs(cmd *cobra.Command) []string {
	...
	if cmd.Flags().Changed("duckdb-path") && strings.TrimSpace(serverDuckDBPath) != "" {
		args = append(args, "--duckdb-path", serverDuckDBPath)
	}
...
func startDetachedServer(args []string) (int, string, error) {
	...
	logPath := filepath.Join(".", "patolake-server.log")
	...
	child.Env = append(os.Environ(), "PATOLAKE_DETACHED=1")
...
func preparePIDFileForStart(pidFile string) error {
	...
	if running {
		return fmt.Errorf("server already running (PID %d); stop it first with `patolake server stop`", pid)
	}
	return nil
}
