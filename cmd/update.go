...
const (
	releasesURL = "https://api.github.com/repos/caioricciuti/pato-lake/releases/latest"
)
...
var updateCmd = &cobra.Command{
	Use:   "update",
	Short: "Update Patolake to the latest version",
	Long:  "Download the latest Patolake release from GitHub and replace the current binary.",
	RunE:  runUpdate,
}
...
func init() {
	updateCmd.Flags().BoolVar(&updateRestartServer, "restart-server", true, "Automatically restart a running Patolake server after update")
	updateCmd.Flags().StringVar(&updatePIDFile, "pid-file", "patolake-server.pid", "Server PID file path used to detect/restart a running server")
...
func runUpdate(cmd *cobra.Command, args []string) error {
	...
	if updateRestartServer {
		...
		if running {
			...
			fmt.Printf("Detected running Patolake server (PID %d); it will be restarted after update.\n", runningPID)
		}
	}
	...
	assetName := fmt.Sprintf("patolake-%s-%s", runtime.GOOS, runtime.GOARCH)
	var assetURL string
	...
	if !updateRestartServer || !running {
		fmt.Println("Restart Patolake to use the new version.")
		return nil
	}

	fmt.Printf("Restarting Patolake server (PID %d)...\n", runningPID)
	...
	if !stopped {
		return fmt.Errorf("binary updated to %s but could not confirm server stop; run `patolake server restart --detach --pid-file %s`", latestTag, updatePIDFile)
	}
...
func sanitizeServerStartArgs(args []string, pidFile string) []string {
	...
	for i < len(args) {
		...
		case strings.HasPrefix(a, "--port=") ||
			...
			strings.HasPrefix(a, "--duckdb-path=") ||
...
func checkWritable(dir string) error {
	tmp := filepath.Join(dir, ".patolake-update-check")
	f, err := os.Create(tmp)
	if err != nil {
		return err
	}
